...
// DELETE method to delete a user (secured with authenticateToken middleware)
...
app.post('/auth', (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query('SELECT user_id, username, password FROM users WHERE username = ? AND password = ?', [username, password], (error, results) => {
        connection.release();
        if (error) {
          console.error('Error executing query:', error.stack);
          return res.status(500).json({ error: 'Internal Server Error' });
        ...
        // No password hashing is done, so just compare plaintext passwords
